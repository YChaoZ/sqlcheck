...
public class ReportGenerator {
    ...
    private void writeFilterScript(PrintWriter writer) {
        ...
        writer.println("                        const matchesSeverity = severities.size === 0 || severities.has(issue.dataset.severity);");
        writer.println("                        const matchesType = types.size === 0 || types.has(issue.dataset.type);");
        writer.println("                        const visible = matchesSeverity && matchesType;");
        writer.println("                        issue.style.display = visible ? 'block' : 'none';");
        writer.println("                        if (visible) { visibleCount++; }");
        writer.println("                    });");
        writer.println("                    fileBlock.style.display = visibleCount > 0 ? 'block' : 'none';");
        writer.println("                });");
        writer.println("            }");
